...
Running it
Python 3.8+, no packages needed to run it.
```bash
python3 src/main.py             
...
Sanitise: normalise Unicode, strip control characters, quarantine over long lines, scan for attack signatures.
Extract: regex finds candidates by shape.
Validate and redact: Python decides whether each candidate is real, then masks it.
...
Eleven threat signatures in XSS, unsafe URI schemes, SQL and template and command injection, path traversal, CRLF, NoSQL operators, null bytes. The sample run catches 15 hostile constructs.
Nothing sensitive appears in the output. Cards show the last 4 digits only. Emails keep two characters and the domain. Hostile payloads are stored as SHA-256 fingerprints are enough to audit a finding, useless to anyone who steals the report.
...
`+250 (0)791 762 771`	12 digits — `(0)` is a trunk prefix
